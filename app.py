"""
app.py — Mistake Analyzer (Hackathon Entry: Meta PyTorch Hackathon)
====================================================================
Defines the global `tasks` list required by the OpenEnv Phase 2
automated validator.  Each task has:
  - name            : str
  - input           : str   (sentence fed to the NLP model)
  - expected_output : value (what a correct model should return)
  - grader          : callable that returns bool | float

FastAPI app is also exposed here so the file doubles as the
application entrypoint when run with `uvicorn app:app`.

Load order (fixes NameError):
  1. imports
  2. grader functions
  3. tasks  (graders already defined — safe to reference)
  4. FastAPI app + routes  (tasks already defined — safe to reference)
  5. entrypoint
"""
...
# ===========================================================================
# FASTAPI SETUP  — defined AFTER tasks so routes can safely reference them
# ===========================================================================
...
try:
    ...
    app = None  # FastAPI not available — validator-only mode


# ===========================================================================
# ENTRYPOINT  (uvicorn app:app  OR  python app.py)
# ===========================================================================

if __name__ == "__main__":
    if app is None:
        print("[ERROR] FastAPI not installed. Install with: pip install fastapi uvicorn", flush=True)
        sys.exit(1)
